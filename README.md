...
- Исправлено управление администраторами: команда `/add_admin` ищет пользователей по Telegram ID, роли отображаются корректно, а список администраторов открывается без ошибок в `app/handlers/admin_full.py`.
- Кнопки возврата в админ-панели теперь редактируют текущее сообщение, а незавершённые разделы вроде истории рассылок отвечают «Функция не настроена», чтобы не оставлять пользователя без реакции.
- Удалено дефолтное сообщение «Выберите дальнейшее действие» в сценариях опросов: текст задаётся только в конфигурации, см. `app/scenes/action_registry.py`.
