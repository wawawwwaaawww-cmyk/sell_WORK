...
- Полностью обновлена 5-вопросная анкета: новые тексты, балльная модель (максимум 13 баллов) и автоматическая рекомендация программы с кнопками записи или запроса деталей, а также расширен сценарий записи на консультацию (кнопки «нет подходящей даты/времени» и ввод вручную).
- Убрано промежуточное сообщение «Отвечай честно, это поможет подобрать программу» перед стартом анкеты — теперь переход сразу к первому вопросу.
- Обновлены тексты вопросов анкеты и форматирование: выделены ключевые строки жирным в стиле Telegram и варианты ответов в кнопках приведены к виду с заглавной буквы.
...
- **Сегментация**: Пользователи делятся на сегменты (COLD 0–4 балла, WARM 5–9 баллов, HOT 10+ баллов) на основе обновлённой анкеты и поведения. Максимальный балл анкеты — 13, что позволяет точнее подбирать релевантный контент.
...
### 📝 Обновлённая анкета и рекомендации

**Описание:**
Анкета состоит из 5 последовательных вопросов и фиксированной шкалы (максимум 13 баллов). Каждый ответ мгновенно сопровождается контекстным комментарием и подводкой к следующему вопросу.

**Структура вопросов и баллы:**
- Опыт в инвестициях: от «нет опыта» (0) до «трейдинг» (2).
- Финансовая цель на год: от «нет цели» (0) до «выйти на пассивный доход» (3).
- Срочность задачи: «просто изучаю» (0) — «в течение месяца» (3).
- Стартовый бюджет: «до 10$» (0) — «от 1000$» (3).
- Готовность инвестировать в обучение: «нет» (0) или «да» (2).

**Результат прохождения:**
- Система формирует профиль и итоговый балл (из 13) и автоматически определяет сегмент.
- На основе ответов подбирается релевантная программа обучения с кнопкой «📘 Узнать про программу».
- При согласии на обучение бот ведёт пользователя через запись на консультацию, при отказе — предлагает бесплатную консультацию и отправку описания программы.

**Расширенное бронирование консультаций:**
- После кнопки «Записаться» пользователь выбирает одну из двух ближайших дат и слот (12:00, 14:00, 16:00, 18:00 МСК).
- На каждом шаге есть кнопка «❌ Нет подходящей даты/времени» для ввода своих вариантов вручную.
- Напоминание отправляется за 15 минут до созвона, при ответе «не приду» бот предлагает перенести встречу с теми же быстрыми кнопками.
